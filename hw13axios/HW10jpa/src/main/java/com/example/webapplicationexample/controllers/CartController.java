...
public class CartController {
    ...
    @GetMapping("/{idClient}")
    public ResponseEntity<List<Product>> getCartsByIdClient(@PathVariable long idClient) {
        log.info("получение корзины клиента по id клиента {}", idClient);
        List<Product> carts = cartService.getProductsInCart(idClient);
    ...
    public ResponseEntity<?> updateProduct(@PathVariable long id,@RequestBody Product product) {
        log.info("обновления продукта {} по id {}", id, product.getName());
